...
const offerHtml = (offer,price) => (`  <div>  
                        <strike> <span class="price-old">${ new Intl.NumberFormat('CLP', {
                            style: 'currency',
                            currency: 'CLP',}).format( price * ( 1 - offer/100 )) 
                        } </span></strike>
...
const paintProducts = ({ data }) => {
    ...
    if (data.length > 0) {
        data.forEach(product => {
            productContainer.innerHTML += `
            <div class="card">   
                <img src="${product.url_image ?? 'https://image.freepik.com/free-vector/store-shelves-with-milk-dairy-products-illustration_1284-52079.jpg'}" alt="" />
                      ...
                      <div>
                        <span class="product-price">
                            ${ new Intl.NumberFormat('CLP', {
                                style: 'currency',
                                currency: 'CLP',}).format(product.price) 
                            }
                        </span>
                      </div>
                        ${ product.discount > 0 ? offerHtml(product.discount,product.price) : "<div></div>" }
                    </span>
                </div>
                <div>
